...
DCDProcess::DCDProcess(int port, QString server, QString client)
{
	m_port = port;
	m_server = server;
	m_client = client;
...
bool DCDProcess::running()
{
	return m_sproc.state() == KProcess::Running;
}


bool DCDProcess::startServer()
{
...
DCDCompletion DCDProcess::complete(QString file, int offset)
{
	KProcess proc;
...
DCDCompletion DCDProcess::complete(QByteArray data, int offset)
{
	KProcess proc;
	proc.setOutputChannelMode(KProcess::MergedChannels);
	proc.setProgram(m_client,
...
DCDCompletion DCDProcess::processCompletion(QString data)
{
...
void DCDProcess::addImportPath(QString path)
{
	addImportPath(QStringList(path));
}

void DCDProcess::addImportPath(QStringList paths)
{
	if(paths.isEmpty()) {
		return;
	}
